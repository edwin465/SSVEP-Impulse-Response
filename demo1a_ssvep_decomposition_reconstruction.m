%% Demo 1a of Decomposition and Reconstruction of SSVEP %%%
...
% In this demo, we decompose an SSVEP and then reconstruct it. We measure
% their difference by using the correlation and MSE. 

% Demo 1a is similar to Demo 1. The difference is that here we use the
% canonical correlation analysis (rather than ALS) to find the spatial
% filter and impulse response when we decompose the SSVEP.
%
...
%             x_hat=x_hat_new;
            ...
            fs1=sti_f(i);
            ph1=pha_val(i);
            ssvep1=squeeze(subband_signal(k).signal_template(i,:,:));                                 % this is the SSVEP we want to reconstruct
            y1=ssvep1;
            y1_oz=ssvep1(oz_ch,:);
